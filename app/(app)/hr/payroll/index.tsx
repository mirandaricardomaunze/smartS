...
export default function PayrollScreen() {
  ...
  const handleProcessAll = () => {
    ...
    useConfirmStore.getState().show({
      ...
      message: `Deseja processar os salários de todos os ${employees.length} funcionários para ${MONTH_NAMES[month - 1]} ${year}?`,
      confirmLabel: 'Processar Agora',
      onConfirm: async () => {
  ...
  const handlePrint = async (item: Payroll, employee: Employee) => {
    feedback.light()
    try {
      await generatePayslipPDF(item, employee, countryConfig)
    } catch {
      useConfirmStore.getState().show({
        title: 'Erro',
        message: 'Não foi possível gerar o recibo PDF.',
        confirmLabel: 'OK',
        showCancel: false,
        onConfirm: () => {}
      })
            ...
            <View className="items-end">
              ...
              <View className="flex-row items-center mt-1">
                <Badge
                  ...
                  label={payslip.status === 'paid' ? 'Pago' : 'Processado'}
                />
                <TouchableOpacity
  ...
  return (
    <Screen padHorizontal={false} withHeader>
      ...
      <View className="px-6 py-4 flex-row items-center space-x-2">
        {/* Navegador de mês */}
        <View className="flex-1 bg-white dark:bg-white/5 rounded-2xl flex-row items-center justify-between px-3 border border-slate-100 dark:border-white/10 h-14">
          <TouchableOpacity
            onPress={handlePrevMonth}
            className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 items-center justify-center"
          >
            <ChevronLeft size={18} color="#6366f1" />
          </TouchableOpacity>

          <Text style={{ fontFamily: 'Inter-Bold' }} className="text-slate-900 dark:text-white font-black text-xs uppercase tracking-widest">
          ...
          <TouchableOpacity
            ...
            className={`w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 items-center justify-center ${isCurrentOrFuture ? 'opacity-30' : ''}`}
          >
      ...
      <View className="px-6 mb-2">
        <View className="flex-row items-center bg-white dark:bg-slate-900 px-4 h-12 rounded-xl border border-slate-100 dark:border-slate-800">
          <Search size={16} color="#6366f1" />
          <TextInput
            placeholder="Pesquisar funcionário..."
            className="flex-1 ml-3 text-slate-700 dark:text-white text-xs"
            value={search}

import { View, Text, TouchableOpacity, ScrollView, Alert } from 'react-native'
import React, { useState, useMemo } from 'react'
import Screen from '@/components/layout/Screen'
import Header from '@/components/layout/Header'
import { 
  ShieldCheck, 
  Calendar, 
  Download, 
  Printer, 
  FileText,
  Users,
  Building2,
  Lock,
  ArrowRight
} from 'lucide-react-native'
import Card from '@/components/ui/Card'
import Badge from '@/components/ui/Badge'
import Loading from '@/components/ui/Loading'
import Button from '@/components/ui/Button'
import { usePayroll } from '@/features/hr/hooks/usePayroll'
import { useFormatter } from '@/hooks/useFormatter'
import { feedback } from '@/utils/haptics'
import { generateINSSReportPDF } from '@/features/hr/utils/hrExportUtils'
import Animated, { FadeInUp } from 'react-native-reanimated'

export default function INSSReportScreen() {
  const [month, setMonth] = useState(new Date().getMonth() + 1)
  const [year, setYear] = useState(new Date().getFullYear())
  const { formatCurrency, getCurrencySymbol } = useFormatter()

  const { getINSSData, lockPeriod, payslips, isLoading } = usePayroll({ month, year })
  const inssData = getINSSData()

  const allPaid = payslips.length > 0 && payslips.every(p => p.status === 'paid')

  const handleLock = () => {
    feedback.heavy()
    Alert.alert(
      'Confirmar Pagamentos',
      `Ao confirmar, todos os ${payslips.length} salários de ${month}/${year} serão marcados como pagos e bloqueados para edição.`,
      [
        { text: 'Cancelar', style: 'cancel' },
        { 
          text: 'Confirmar e Bloquear', 
          onPress: async () => {
            await lockPeriod()
            feedback.success()
            Alert.alert('Sucesso', 'Mês fechado com sucesso!')
          } 
        }
      ]
    )
  }

  const handleExportPDF = async () => {
    feedback.medium()
    try {
      if (!inssData) return
      await generateINSSReportPDF(inssData, month, year, getCurrencySymbol())
    } catch (error: any) {
      Alert.alert('Erro', 'Não foi possível gerar a guia INSS.')
    }
  }

  if (isLoading || !inssData) return <Loading />

  return (
    <Screen padHorizontal={false} className="bg-slate-50 dark:bg-slate-950 flex-1" withHeader>
      <Header title="Declaração INSS" />

      <ScrollView className="flex-1" contentContainerClassName="pb-32 px-6">
        {/* Month Selector Placeholder (Visual only for now) */}
        <Card variant="glass" className="p-4 flex-row items-center justify-between mb-6 mt-4">
          <View className="flex-row items-center">
            <Calendar size={20} color="#4f46e5" className="mr-3" />
            <Text style={{ fontFamily: 'Inter-Bold' }} className="text-slate-900 dark:text-white font-black text-sm uppercase">
              Período: {month}/{year}
            </Text>
          </View>
          <Badge variant={allPaid ? 'success' : 'warning'} label={allPaid ? 'Fechado' : 'Aberto'} />
        </Card>

        {/* Global Stats */}
        <Animated.View entering={FadeInUp} className="space-y-4">
          <Card variant="premium" className="p-5 border-slate-100 dark:border-white/5 bg-indigo-500">
             <View className="flex-row justify-between items-center">
               <View>
                 <Text className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-1">Total INSS a Pagar (8%)</Text>
                 <Text style={{ fontFamily: 'Inter-Black' }} className="text-3xl text-white font-black">
                   {formatCurrency(inssData.total_inss)}
                 </Text>
               </View>
               <View className="bg-white/10 p-3 rounded-2xl">
                 <ShieldCheck size={32} color="white" />
               </View>
             </View>
          </Card>

          <View className="flex-row">
             <Card variant="premium" className="flex-1 p-4 border-slate-100 dark:border-white/5 mr-3">
                <Text className="text-slate-400 text-[10px] font-bold uppercase mb-1">Trabalhador (4%)</Text>
                <Text style={{ fontFamily: 'Inter-Bold' }} className="text-slate-900 dark:text-white text-base">
                  {formatCurrency(inssData.total_inss_employee)}
                </Text>
             </Card>
             <Card variant="premium" className="flex-1 p-4 border-slate-100 dark:border-white/5">
                <Text className="text-slate-400 text-[10px] font-bold uppercase mb-1">Empresa (4%)</Text>
                <Text style={{ fontFamily: 'Inter-Bold' }} className="text-slate-900 dark:text-white text-base">
                  {formatCurrency(inssData.inss_employer)}
                </Text>
             </Card>
          </View>
        </Animated.View>

        {/* Breakdown */}
        <Text style={{ fontFamily: 'Inter-Bold' }} className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight mb-4 mt-8 ml-1">
          Resumo Estrutural
        </Text>

         <Card variant="premium" className="p-5 border-slate-100 dark:border-white/5">
            <View className="flex-row justify-between items-center border-b border-slate-50 dark:border-white/5 pb-3 mb-4">
              <View className="flex-row items-center">
                 <Users size={16} color="#64748b" className="mr-3" />
                 <Text className="text-slate-600 dark:text-slate-300 font-medium">Funcionários Processados</Text>
              </View>
              <Text className="text-slate-900 dark:text-white font-black">{inssData.employee_count}</Text>
           </View>

           <View className="flex-row justify-between items-center border-b border-slate-50 dark:border-white/5 pb-3">
              <View className="flex-row items-center">
                 <Building2 size={16} color="#64748b" className="mr-3" />
                 <Text className="text-slate-600 dark:text-slate-300 font-medium">Massa Salarial Base</Text>
               </View>
               <Text className="text-slate-900 dark:text-white font-black">{formatCurrency(inssData.total_base)}</Text>
            </View>
            <View className="flex-row justify-between items-center mt-4">
              <View className="flex-row items-center">
                 <FileText size={16} color="#64748b" className="mr-3" />
                 <Text className="text-slate-600 dark:text-slate-300 font-medium">Total IRPS (Retenção)</Text>
              </View>
              <Text className="text-slate-900 dark:text-white font-black">{formatCurrency(inssData.total_irps)}</Text>
           </View>
        </Card>

         {/* Actions */}
         <View className="mt-8">
            {!allPaid ? (
              <Button 
                title="Fechar Mês e Confirmar Pagamentos" 
                icon={<Lock size={18} color="white" />}
                onPress={handleLock}
                variant="primary"
                className="mb-3"
              />
           ) : (
             <View className="bg-emerald-500/10 p-4 rounded-2xl flex-row items-center border border-emerald-500/20 mb-3">
                <ShieldCheck size={20} color="#10b981" className="mr-3" />
                <Text className="text-emerald-700 dark:text-emerald-400 font-bold text-sm">Este período foi fechado e os recidos bloqueados.</Text>
             </View>
           )}

           <Button 
             title="Exportar Guia INSS (PDF)" 
             icon={<Download size={18} color="#4f46e5" />}
             onPress={handleExportPDF}
             variant="ghost"
             className="border border-primary/20"
           />
        </View>
      </ScrollView>
    </Screen>
  )
}

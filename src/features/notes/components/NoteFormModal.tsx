...
import { Product, NoteType, NoteItem } from '@/types'
...
interface NoteFormModalProps {
  visible: boolean
  onClose: () => void
  onSave: (data: { number: string; type: NoteType; items: NoteItem[] }) => Promise<void>
}
              ...
              {items.length === 0 ? (
                ...
                <View className="gap-3">
                  {items.map((item, index) => {
                    ...
                    return (
                      <Card key={item.product_id} variant="glass" glassIntensity={10} className="p-4 border-slate-100 dark:border-slate-800">
